---
layout: '../../layouts/BaseLayout.astro'
title: Card
order: 1
---

# Card

## Usage

There is no card component, only a CSS class

<div class="p-4 bg-white flex flex-wrap gap-4 border border-gray-100 rounded-lg my-4">
  <div class="card h-20 w-32 flex items-center justify-center">hello</div>
  <div class="card h-20 w-32 flex items-center justify-center">goodbye</div>
</div>

```html
<div class="p-4 bg-white flex flex-wrap gap-4">
  <div class="card h-20 w-32 flex items-center justify-center">hello</div>
  <div class="card h-20 w-32 flex items-center justify-center">goodbye</div>
</div>
```

One can also add more shadow for a marketing card

`shadow-marketing-card`

<div class="p-4 bg-white flex flex-wrap gap-4 border border-gray-100 rounded-lg my-4">
  <div class="card shadow-marketing-card h-20 w-32 flex items-center justify-center">hello</div>
  <div class="card shadow-marketing-card h-20 w-32 flex items-center justify-center">goodbye</div>
</div>

```html
<div class="p-4 bg-white flex flex-wrap gap-4">
  <div class="card shadow-marketing-card h-20 w-32 flex items-center justify-center">
    hello
  </div>
  <div class="card shadow-marketing-card h-20 w-32 flex items-center justify-center">
    goodbye
  </div>
</div>
```

`shadow-marketing-card` adds a heavier drop shadow appropriate for marketing pages.
