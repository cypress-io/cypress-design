...
**Build the problem-first check into the agent, not just the people.** Challenging a solution-first brief — what problem does this solve, whose job does it serve? — is everyone's job: PM, design, and agents alike (see "Start from the problem, not the solution" in ux.md and "Push back when the framing is wrong" in feedback.md). The move specific to AI is encoding that check into the tooling, so the agent asks before generating instead of relying on someone to remember. Whatever the brief is called — product brief, design brief, problem statement, discovery doc — if it leads with what to build, surface the missing problem first.
