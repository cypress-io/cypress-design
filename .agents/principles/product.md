...
**A dedicated UI is one candidate solution, not the default one — and it's the one everyone reaches for first.** When a problem needs solving, a UI is usually the first option that comes to mind, and often the only one anyone writes down, not because it's the best fit but because it's the shape of solution everyone already knows how to picture. Docs, config-as-code, a CLI, an MCP or agent-facing tool solve the same class of problem — routing, filtering, delivery, configuration — often cheaper to build and easier to extend, and they rarely make it onto the list of options compared. Before committing to build a dedicated interface, name at least one non-UI alternative and give it a real hearing in the opportunity-space comparison above; build the UI because it won that comparison, not because it's what came to mind first.
