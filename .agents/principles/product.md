...
**Assume UI is the most expensive option — because it usually is.** A dedicated interface costs design, engineering, QA, and upkeep for as long as it exists. Docs, config-as-code, a CLI, or an MCP/agent-facing tool solve the same class of problem — routing, filtering, delivery, configuration — for a fraction of that cost, and they rarely make it onto the list of options compared, because UI is the default shape of solution everyone already knows how to picture. Before committing to build a dedicated interface, name at least one non-UI alternative and give it a real hearing in the opportunity-space comparison above — build the UI because it won on cost, not because it's what came to mind first.
