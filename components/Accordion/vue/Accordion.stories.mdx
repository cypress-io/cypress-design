import { ArgsTable, Canvas, Meta, Story, Description } from '@storybook/addon-docs'
import { version } from "./package.json"
import changelog from "./CHANGELOG.md?raw"
import { FIGMA_URL } from '../constants'

import Accordion from './Accordion.vue'

import AccordionStory from './Accordion.rootstory'
import { IconActionQuestionMarkCircle } from '@cypress-design/vue-icon'

<h1>Accordion<span className="text-lg font-normal"> - v{version}</span></h1>

<Meta title="Accordion" component={Accordion} parameters={{
    design: {
      type: "figma",
      url: FIGMA_URL,
    }
  }} />

  
### Usage

```tsx
import Accordion from '@cypress-design/vue-accordion'
```

<Canvas withSource="none">
  <Story name="Accordion">
    {AccordionStory()}
  </Story>
</Canvas>

```html
<Accordion
  :icon="IconActionQuestionMarkCircle"
  title="Accordion Title"
  description="Vestibulum id ligula porta felis euismod semper. Nulla... "
  separator
>
  Lorem ipsum, dolor sit amet consectetur adipisicing elit. Odio dolore omnis
  nemo minus, sapiente magni ...
</Accordion>
```

### Without separator

<Canvas withSource="none">
  <Story name="No Separator">
    {AccordionStory({separator:false})}
  </Story>
</Canvas>

```html
<Accordion
  :icon="IconActionQuestionMarkCircle"
  title="Accordion Title"
  description="Vestibulum id ligula porta felis euismod semper. Nulla... "
>
  Lorem ipsum, dolor sit amet consectetur adipisicing elit. Odio dolore omnis
  nemo minus, sapiente magni ...
</Accordion>
```

### Without description

<Canvas withSource="none">
  <Story name="No Description">
    {AccordionStory({description:null})}
  </Story>
</Canvas>

```html
<Accordion
  :icon="IconActionQuestionMarkCircle"
  title="Accordion Title"
  separator
  open
>
  Lorem ipsum, dolor sit amet consectetur adipisicing elit. Odio dolore omnis
  nemo minus, sapiente magni ...
</Accordion>
```

### Open by default

<Canvas withSource="none">
  <Story name="Open">
    {AccordionStory({open:true})}
  </Story>
</Canvas>

```html
<Accordion
  :icon="IconActionQuestionMarkCircle"
  title="Accordion Title"
  description="Vestibulum id ligula porta felis euismod semper. Nulla... "
  separator
  open
>
  Lorem ipsum, dolor sit amet consectetur adipisicing elit. Odio dolore omnis
  nemo minus, sapiente magni ...
</Accordion>
```

### Icon slot

If you need classes or props for your icon, you can pass it in via the `iconEl` slot instead of the `icon` prop.

<Canvas withSource="none">
  <Story name="Icon slot">
    {AccordionStory({iconEl: true})}
  </Story>
</Canvas>

```html
<Accordion
  title="Accordion Title"
  description="Vestibulum id ligula porta felis euismod semper. Nulla... "
  separator
  open
>
  Lorem ipsum, dolor sit amet consectetur adipisicing elit. Odio dolore omnis
  nemo minus, sapiente magni ...
  <template #iconEl>
    <IconActionQuestionMarkCircle
      strokeColor="jade-600"
      fillColor="jade-50"
    />
  </template>
</Accordion>
```

<ArgsTable />

<Description>
  {changelog.replace(/^# .+/, '## Changelog')}
</Description>

