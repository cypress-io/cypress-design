      ...
      >
        {/* Show "No results" only when an active search filter left zero
            *selectable* rows. `searchValue` gates out the false-positive
            ...
            `selectableIndices` (not `interactiveIndices`) is
            deliberate: the filter keeps standalone divider and button
            rows (so a "+ Add new" action stays visible during search),
            and those rows shouldn't suppress the banner — the user
            still has nothing they can actually pick. A disabled row
            that matched the query by label counts as a visible-but-
            unselectable result and (accepted trade-off) shows the
            banner alongside it. */}
        {searchValue && selectableIndices.length === 0 && (
