...
How the RunStatus component is built. Read this when extending, fixing, or rebuilding the component. For usage, see [`instructions.md`](./instructions.md). The migration plan from the cypress-services `RunStats` source lives in [`plan.md`](./plan.md).
...
├── plan.md                   # Cypress-services → cypress-design port plan
...
Ported from `cypress-io/cypress-services` — `frontend/packages/design-system/src/components/RunStats.tsx` + `module.RunStats.scss`. The port drops application coupling (router, query-string builder, `data-pendo` analytics, project/build URL construction) and remaps SCSS variables to Tailwind utility classes. See [`plan.md`](./plan.md) for the full delta.
...
| Status       | Icon component                                                                 | Source                             |
| ------------ | ------------------------------------------------------------------------------ | ---------------------------------- |
| `passed`     | `<StatusIcon status="passed" />`                                               | `@cypress-design/react-statusicon` |
| `failed`     | `<StatusIcon status="failed" />`                                               | same                               |
| `skipped`    | `<StatusIcon status="skipped" />`                                              | same                               |
| `pending`    | `<StatusIcon status="pending" />`                                              | same                               |
| `flaky`      | `<IconStatusFlaky />`                                                          | `@cypress-design/react-icon`       |
...
A small helper `<Link>` (React) / `<Link>` template fragment (Vue) decides between `renderLink(href, children)`, `<a href={href}>{children}</a>`, and "no link":

```tsx
function StatLink({ href, renderLink, children, ...rest }) {
  if (!href) return <span {...rest}>{children}</span>
  if (renderLink) return renderLink(href, children) // caller owns wrapping <a>
  return (
    <a href={href} {...rest}>
      {children}
    </a>
...
`renderLink` is a unified **function prop** in both React and Vue — not a Vue scoped slot. The signature is `(href: string, children: VNode | ReactNode) => unknown`. The `children` argument is whatever the framework renders for the inner icon + count (a JSX/TSX element in React, a VNode in Vue). Consumers wrap it in their framework's router link.

The `aria-label` and `data-cy="link-{status}"` must end up on the rendered element. When `renderLink` is used, the caller is responsible — document this in `instructions.md` (it already is). When the native `<a>` path is used, the component applies both.
