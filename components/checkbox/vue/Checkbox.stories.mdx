import { ArgsTable, Canvas, Meta, Story, Description } from '@storybook/addon-docs'
import { version } from "./package.json"
import changelog from "./CHANGELOG.md?raw"

import Checkbox from './Checkbox.vue'

<h1>Checkbox<span className="text-lg font-normal"> - v{version}</span></h1>

<Meta title="Checkbox" component={Checkbox}
  argTypes={{
    color: {
      options: ['indigo', 'red', 'jade'],
      control: {
        type: 'select',
      },
    },
  }}
  parameters={{
    design: {
      type: "figma",
      url: 'https://www.figma.com/file/1WJ3GVQyMV5e7xVxPg3yID/Design-System?node-id=1027%3A9825',
    }
  }} />

<Canvas>
  <Story name="Checkbox">
    {({ color }) => ({ 
      template: `
        <Checkbox :modelValue="true" label="Checked" :color="color"/>
        <br />
        <Checkbox :modelValue="false" label="Unchecked" :color="color"/>
        <br />
        <Checkbox :modelValue="true" disabled label="Disabled & checked" :color="color"/>
        <br />
        <Checkbox :modelValue="false" disabled label="Disabled & unchecked" :color="color"/>
      `,
      components: { Checkbox },
      data: () => ({
        color
      })
    })}
  </Story>
</Canvas>

<ArgsTable />

<Description>
  {changelog.replace(/^# .+/, '## Changelog')}
</Description>
