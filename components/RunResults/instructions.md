...
| `renderLink`     | `(href: string, children: unknown) => unknown` | —         | Custom link renderer. Same signature in both frameworks; `children` is the framework's native rendered icon + count (a JSX element in React, an **array** of `VNode`s in Vue), typed as `unknown` because the two shapes differ — narrow it inside your callback if you need typed access. Falls back to a native `<a href>` when not provided. |
