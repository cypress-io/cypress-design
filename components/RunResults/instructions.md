...
- **Run-status pill:** the linked build-number segment renders an `<a>` with `aria-label="View run #{buildNumber} — {status label}"` (e.g. `"View run #468 — Passed"`). Folding the status word into the aria-label is deliberate: screen readers announce `aria-label` in place of the visible `#468` text, so without the status the reader would never convey whether the run passed, failed, or is running. The branch segment (if present) is always plain text — no link, no `aria-label`. The pill carries a `title="{status label}"` attribute as a hover-tooltip fallback for pointer users (the linked segment's `aria-label` supersedes it for AT). The divider between segments is a CSS `::after` pseudo-element on the first segment — it has no DOM node and is invisible to assistive technology by construction (no ARIA needed).
