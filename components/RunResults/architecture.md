...
For the test-counts pill, the `aria-label` and `data-cy="link-{status}"` end up on the rendered element. For the run-status pill's build-number segment, `aria-label="View run #{buildNumber} — {RUN_STATUS_LABELS[status]}"` (the readable status word is folded in because screen readers replace the visible `#468` with the aria-label — without it, AT users never hear the status) and `data-cy="run-status-build-number"`. The branch segment is always a plain `<span>` (never a link) with `data-cy="run-status-branch"` and no `aria-label`. The native `<a>` path applies aria-label + data-cy; the `renderLink` path delegates aria-label responsibility to the caller (noted in `instructions.md`).
