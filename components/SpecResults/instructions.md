...
- Three pills carry an explanatory tooltip on hover/focus (`@cypress-design/react-tooltip`, dark). `skipped` _always_ breaks `skipped`/`cancelled` back into "N specs skipped with no tests" / "N specs skipped via Auto Cancellation" — "skipped" alone never says which real status a spec actually has, so the reason is named even when only one cause contributes. The remaining pill breaks `running`/`queued` into "N specs running" / "N specs queued", but only when _both_ are nonzero — "N specs remaining" is already fully explained on its own when every remaining spec is in the same state. The scheduled-to-complete pill always gets a one-line explanation that the delay is a project setting.
