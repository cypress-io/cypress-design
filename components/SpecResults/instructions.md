...
- **Complete with zero counts:** every count is zero, but the caller passes `isComplete` (e.g. a `NOTESTS` run where no specs were ever claimed). No pills render and the bar stays empty -- zero is treated as the real, final count rather than "not known yet". The caller's `description` slot is expected to carry the actual explanation (e.g. "no spec files matched your `specPattern`").
